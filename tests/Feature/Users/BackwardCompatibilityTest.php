...
test('user can still be found by integer id', function (): void {
    ...
    expect($found)->not->toBeNull()->and($found?->id)->toBe($user->id)->and($found?->ulid)->toBe($user->ulid);
});

test('user has both id and ulid', function (): void {
    $user = User::factory()->create();

    expect($user->id)->toBeInt()->and($user->ulid)->toBeString()->and(mb_strlen($user->ulid))->toBe(26);
});

test('user primary key is still integer id', function (): void {
    $user = new User();

    expect($user->getKeyName())->toBe('id')->and($user->getRouteKeyName())->toBe('ulid'); // Route uses ULID
});
...
test('user can be queried by id for foreign key relationships', function (): void {
    ...
    $found = User::query()->where('id', $user->id)->first();

    expect($found)->not->toBeNull()->and($found?->ulid)->toBe($user->ulid);
});
