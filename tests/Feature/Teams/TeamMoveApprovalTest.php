...
it('creates approval request when move requires approval due to descendant threshold', function (): void {
    ...
    Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    ...
    expect($result)
        ->toBeInstanceOf(TeamMoveApproval::class)
        ->and($result->status)
        ->toBe('pending')
        ->and($result->team_id)
        ->toBe($org->id)
        ->and($result->from_parent_id)
        ->toBe($enterprise->id)
        ->and($result->to_parent_id)
        ->toBeNull()
        ->and($result->requested_by_id)
        ->toBe($user->id)
        ->and($result->reason)
        ->toBe('Reason for move');
});
...
it('creates approval request for cross-organisation move', function (): void {
    ...
    $division = Division::factory()->create(['parent_id' => $org1->id, 'tenant_id' => $enterprise->id]);

    $user = User::factory()->create(['tenant_id' => $enterprise->id]);

    $result = $this->service->requestMove($division, $org2->id, $user);

    expect($result)->toBeInstanceOf(TeamMoveApproval::class)->and($result->status)->toBe('pending');
});
...
it('approves move request when all required approvers approve', function (): void {
    ...
    Role::query()->firstOrCreate(['name' => 'organisation_admin', 'guard_name' => 'web']);
    ...
    $approval->refresh();
    expect($approval->status)->toBe('approved')->and($approval->approved_at)->not->toBeNull();
...
it('rejects move request', function (): void {
    ...
    expect($approval->status)
        ->toBe('rejected')
        ->and($approval->rejected_at)
        ->not->toBeNull()->and($approval->rejected_by_id)->toBe($approver->id)->and($approval->rejection_reason)->toBe(
            'Not a good idea',
        );
...
it('creates approval request when depth change exceeds threshold', function (): void {
    ...
    $org = Organisation::factory()->create(['parent_id' => $enterprise->id, 'tenant_id' => $enterprise->id]);
    $division = Division::factory()->create(['parent_id' => $org->id, 'tenant_id' => $enterprise->id]);
    $dept = Department::factory()->create(['parent_id' => $division->id, 'tenant_id' => $enterprise->id]);
