...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['model_has_permissions'], static function (Blueprint $table) use (
            ...
            $table->foreign($pivotPermission)->references('id')->on($tableNames['permissions'])->onDelete('cascade'); // permission id
            if ($teams) {
        ...
        Schema::create($tableNames['model_has_roles'], static function (Blueprint $table) use (
            ...
            $table->foreign($pivotRole)->references('id')->on($tableNames['roles'])->onDelete('cascade'); // role id
            if ($teams) {
                $table->unsignedBigInteger($columnNames['team_foreign_key']);
