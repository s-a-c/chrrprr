...
OUTPUT=$(DB_CONNECTION=sqlite DB_DATABASE=:memory: PEST_MUTATE=1 php -d error_reporting="E_ALL & ~E_DEPRECATED & ~E_USER_DEPRECATED" -d memory_limit=1G vendor/bin/pest --mutate --parallel --min=90 --covered-only --default-time-limit=10 2>&1) || EXIT_CODE=$?
...
FILTERED_OUTPUT=$(echo "$OUTPUT" | awk '
    BEGIN {
